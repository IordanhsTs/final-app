...
// ⚠️ ΠΡΕΠΕΙ ΝΑ ΕΙΝΑΙ ΠΑΝΤΑ ΙΔΙΟ με το channel id στο App.js — αλλιώς το Firebase
// δεν βρίσκει το κανάλι στη συσκευή και ρίχνει την ειδοποίηση στο δικό του
// εφεδρικό «Miscellaneous», με τον γενικό ήχο του κινητού. Το σύμπτωμα είναι
// ύπουλο: η ειδοποίηση ΦΤΑΝΕΙ κανονικά, απλώς με λάθος ήχο, οπότε μοιάζει με
// πρόβλημα ήχου ενώ είναι αναντιστοιχία ονόματος.
//
// Τα κανάλια είναι ΑΜΕΤΑΒΛΗΤΑ στο Android: κάθε αλλαγή αρχείου ήχου απαιτεί νέο
// id (_v2 → _v3 → …) ΚΑΙ στα δύο σημεία, μαζί με νέο build. Και επειδή το νέο
// build σβήνει τα προηγούμενα κανάλια στην πρώτη εκκίνηση, το deploy εδώ πρέπει
// να γίνεται τη στιγμή που μπαίνει το APK στα κινητά — όχι πριν, όχι μετά.
const ASSIGNMENT_CHANNEL = 'assignments_urgent_v3'
